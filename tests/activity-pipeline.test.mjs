...
test("produceActivityArtifact gives the model grouped, constrained input and publishes only validated output", async () => {
  let receivedInput;
  const adapter = {
    async summarize(input) {
      receivedInput = input;
      return {
        items: [safeCandidate, { ...safeCandidate, title: "fictional-client work" }],
      };
    },
  };

  const artifact = await produceActivityArtifact({
    activity: eligibleSource,
    asOf: "2026-08-23",
    authorLogin: "bradley",
    repositoryAllowlist: ["fictional-service"],
    adapter,
  ...
  assert.equal(receivedInput.groups.length, 2);
  assert.equal(artifact.items.length, 1);
  assert.equal(artifact.items[0].title, safeCandidate.title);
});

test("produceActivityArtifact fails closed when the model response is malformed", async () => {
  await assert.rejects(
    () => produceActivityArtifact({
      ...
      authorLogin: "bradley",
      repositoryAllowlist: ["fictional-service"],
      adapter: { async summarize() { return { unexpected: "shape" }; } },
      denylist: DENYLIST,
    }),
    /candidate item array/,
  );
});

test("produceActivityArtifact publishes an empty safe artifact without eligible source activity", async () => {
  let called = false;
  const artifact = await produceActivityArtifact({
    activity: [],
    asOf: "2026-08-23",
...
test("createHostedLanguageModelAdapter requires an explicitly approved privacy policy", async () => {
  assert.throws(
    () => createHostedLanguageModelAdapter({
      provider: "fictional-provider",
      endpoint: "https://model.example/summarize",
      apiKey: "secret",
      policy: {
        approved: true,
        noTraining: false,
  ...
  const result = await adapter.summarize({
    groups: [{
      theme: ["Streaming"],
      events: [{ date: "2026-08-23", title: "Safe", description: "Summary" }],
    }],
  });

  assert.deepEqual(result, { items: [safeCandidate] });
  assert.equal(request.input.groups[0].events[0].title, "Safe");
  assert.equal(request.provider, "fictional-provider");
});
